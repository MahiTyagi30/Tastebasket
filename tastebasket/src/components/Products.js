    ...
    <div className="box-container">
          {ProductData.map((Products) => (
            ...
            <div key={Products.id} className="box">
              ...
              <p>{Products.price}</p>
              <button
      className={`wishlist-button ${wishlist.some((item) => item && item.id === Products.id) ? 'added' : ''}`}
      onClick={() => {
        wishlist.some((item) => item && item.id === Products.id)
          ? removeFromWishlistHandler(Products.id)
          : addToWishlistHandler(Products);
      }}
    >
      {wishlist.some((item) => item && item.id === Products.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
    </button>
          ...
          >
            {wishlist.some((item) => item.id ===Products.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
              ...
              <p><StarRating rating={Products.rating} /></p>
             
                  
                
              <Button variant="primary" onClick={() => { send(Products) }} className="col-lg-12">
                  Add to Cart
                </Button>
            </div>
          ))}

...
export default function Products() {
  ...
  return (
    ...
    <div className="box-container">
          { Sweets.map((Sweets) => (
            <div key={Sweets.id} className="box">
              <img src={Sweets.image} alt={Sweets.name} />
              <h3>{Sweets.name}</h3>
              <p>{Sweets.price}</p>
              <button
      className={`wishlist-button ${wishlist.some((item) => item && item.id === Sweets.id) ? 'added' : ''}`}
      onClick={() => {
        wishlist.some((item) => item && item.id === Sweets.id)
          ? removeFromWishlistHandler(Sweets.id)
          : addToWishlistHandler(Sweets);
      }}
    >
      {wishlist.some((item) => item && item.id ===Sweets.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
    </button>
              {/* <button
          ...
          >
            {wishlist.some((item) => item.id ===Products.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
              ...
              <p> <StarRating rating={Sweets.rating} /></p>
              <Button variant="primary" onClick={() => { send(Sweets) }} className="col-lg-12">
                  Add to Cart
                </Button>
            </div>
          ))}

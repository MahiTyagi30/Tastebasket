...
export default function Products() {
  ...
  return (
    ...
    <div className="box-container">
          { Fruits.map((Fruits) => (
            <div key={Fruits.id} className="box">
              <img src={Fruits.image} alt={Fruits.name} />
              <h3>{Fruits.name}</h3>
              <p>{Fruits.price}</p>
              <button
      className={`wishlist-button ${wishlist.some((item) => item && item.id === Fruits.id) ? 'added' : ''}`}
      onClick={() => {
        wishlist.some((item) => item && item.id === Fruits.id)
          ? removeFromWishlistHandler(Fruits.id)
          : addToWishlistHandler(Fruits);
      }}
    >
      {wishlist.some((item) => item && item.id ===Fruits.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
    </button>
              {/* <button
          ...
          >
            {wishlist.some((item) => item.id ===Products.id) ? 'Remove from Wishlist' : 'Add to Wishlist'}
              ...
              <p> <StarRating rating={Fruits.rating} /></p>
              <Button variant="primary" onClick={() => { send(Fruits) }} className="col-lg-12">
                  Add to Cart
                </Button>
            </div>
          ))}
